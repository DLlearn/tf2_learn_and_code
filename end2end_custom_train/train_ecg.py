...
def print_metrics(labels, predictions, target_names, save = False, save_path = None, epoch = None, train_time = None,
    ...
    result_report = 'Epoch:{} with train_time:{:2}min and test_time:{:2}min\n' \
...
def plot_confusion_matrix(cm, class_names):
    ...
    for i, j in itertools.product(range(cm.shape[0]), range(cm.shape[1])):
        color = "white" if cm.iloc[i, j] > threshold else "black"
...
def main():
    ...
    def train_on_batch(model,optimizer,datas,labels,
        ...
        train_epoch_total_loss(total_loss)
        return gradients
    ...
    train_epoch_total_loss = tf.keras.metrics.Mean()
    ...
    val_loss = []
    format_str = (
        '%s:  step:%-7d epoch:%-7.4f/%d celoss:%-6.2f regloss:%-6.2f total_loss:%-6.2f train_batch_acc:%-5.2f%% (%.1f examples/sec; %-4.3f sec/batch)')
    ...
    for epoch in range(args.epochs):
        ...
        start = time.time()
        for i, (data, label) in enumerate(train_ds.take(1000)):
            ...
            if (i + 1) % 50 == 0:
                ...
                current_epoch = (i + 1) / ((epoch + 1) * train_steps_per_epoch)
                ...
                with train_summary_writer.as_default():  # 每50步记录一下，太频繁会影响训练速度
                    ...
                    tf.summary.scalar('train_epoch_total_loss', train_epoch_total_loss.result(), step = step)
                    train_summary_writer.flush()

            if ((i + 1) % int(train_steps_per_epoch * 0.01))==0:
                step = tf.constant(epoch * train_steps_per_epoch + i + 1)
        ...
        print("Training Epoch:{}/{} loss:{:.4f} acc:{:.4f} fineshed usetime:{:.1f} sec".format(epoch + 1,
                                                                                               args.epochs,
                                                                                               train_epoch_total_loss.result().numpy(),
                                                                                               train_epoch_acc.result().numpy(),
                                                                                               end))


        train_acc.append(train_epoch_acc.result().numpy())
        train_loss.append(train_epoch_total_loss.result().numpy())
        train_epoch_acc.reset_states()
        train_epoch_total_loss.reset_states()
        #完成一个epoch的训练
        train_summary_writer.flush()
        # 对测试集进行测试
        ...
        # for (data, label) in tqdm.tqdm(validation_ds.take(1000),total=1000):
